...
function slowed_toggle() {
    if(playlist[playing].hasOwnProperty('song_file_slowed')){
        ...
        audio.onloadedmetadata = function() {
            audio.currentTime = Math.floor((audio.duration / duration) * current);
            audio.play();
...
function play_from_playlist(id) {
    ...
    if(playlist[id].hasOwnProperty('song_file_slowed')) {
        src = slowed ? playlist[id].song_file_slowed : playlist[id].song_file;
        song_name = slowed ? playlist[id].name + " (𝒔𝒍𝒐𝒘𝒆𝒅 𝒏 𝒓𝒆𝒗𝒆𝒓𝒃)" : playlist[id].name;
    } else {
        src = playlist[id].song_file;
        song_name = playlist[id].name;
    }
